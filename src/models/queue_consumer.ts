import { logstr, QueueConsumerLockKey } from '../constants';
import { RefId } from '../utilities';

const criticalPointKey = QueueConsumerLockKey;
...
// LEASE_MS instead of holding it for up to a 90-minute rest. LEASE_MS comfortably exceeds the
// one-minute floor Chrome imposes on background-tab timers.
...
export class QueueConsumer {
	...
	func: () => Promise<void>;
	canRun: () => boolean;
	private _timeout: number | null; // lock poll while running, or the cooldown timer while stopped
	private _func_timeout: number | null; // the one pending block
	private _heartbeat: number | null; // lease renewal while we hold the lock
	private _running: boolean;
	private _waiting: boolean; // true while another tab holds the lock (for one-shot logging)
	...
	*/
		...
		do {
			...
			if (!cp || cp.refId === this._refId || cp.time <= new Date().valueOf()) {
				...
				await this.storage.set({
					[criticalPointKey]: {
						refId: this._refId,
						time: new Date().valueOf() + LEASE_MS,
					},
				});
	...
	// extend our lease. returns false if another tab has taken the lock in the meantime
	// (possible if this tab was throttled long enough for the lease to lapse).
	private async renewLease(): Promise<boolean> {
		...
		await this.storage.set({
			[criticalPointKey]: { refId: this._refId, time: new Date().valueOf() + LEASE_MS },
		});
		return true;
	}
	...
	async sync() {
		...
		if (!acquired || !this.canRun()) {
			if (!this._waiting) {
				this._waiting = true;
				console.log(logstr, 'another tab is blocking; this tab will wait its turn');
			}
		...
		if (this._waiting) {
			this._waiting = false;
			console.log(logstr, 'this tab now holds the block lock');
		}
		if (this._heartbeat === null) {
			this._heartbeat = setInterval(async () => {
				if (await this.renewLease()) return;
				// we lost the lock: drop the pending block and go back to waiting our turn
				console.log(logstr, 'lost the block lock to another tab');
				this.stopHeartbeat();
		...
		console.log(logstr, `next block in ${Math.round(delay / 1000)}s`);
			...
			this.func()
				.then(() => {
					...
					if (this._blocksThisSession >= SESSION_CAP) {
						this._blocksThisSession = 0;
						console.log(logstr, 'session block cap reached, pausing consumer');
	...
	cooldown(min: number = COOLDOWN_MIN_MS, max: number = COOLDOWN_MAX_MS) {
		...
		console.log(logstr, `queue consumer cooling down for ${Math.round(wait / 60_000)}m`);
		this._timeout = setTimeout(() => {
			this._timeout = null;
			this.start();
		}, wait);
	...
	start() {
		...
		this._running = true;
		console.log(logstr, 'queue consumer started');
		this.sync();
	}
	stop() {
		this._running = false;
		...
		console.log(logstr, 'queue consumer stopped (queue empty or halted)');
	}
}
